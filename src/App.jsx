...
export default function RegaloSorpresa() {
  const opcionesBase = [
    ...
    { id: 6, titulo: 'Un escape room', descripcion: 'En estos juegos hay que ser estratégico. Está comprobado que si te comes un plato de cocido antes de entrar, el grupo pasa de tardar en hacer los puzzles 5 minutos a completarlos en 30 segundos con tal de huir de la peste. Perfecto para poner a prueba vuestra inteligencia y capacidad de aguante... aunque no será necesario, porque no es este 😅' },
      ...
      {!seleccion ? (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-md w-full">
          <h1 className="text-2xl font-semibold mb-4 text-pink-700">
            Bueno, después de la consulta popular hemos decidido colectivamente que no queremos haceros sufrir tanto...
          </h1>
          <p className="mb-6 text-gray-800">
            Así que os vamos a decir cuál es el regalo. Las opciones que habíamos barajado eran las siguientes:
          </p>
          <div className="grid gap-3">
            {opciones.map((op) => {
              if (op.id === 11 && !esActivaOpcion11) return null;
              const estiloExtra =
                op.id === 11 && esActivaOpcion11
                  ? 'bg-gradient-to-r from-yellow-400 to-red-500 text-2xl font-bold'
